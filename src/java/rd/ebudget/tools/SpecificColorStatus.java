...
public class SpecificColorStatus {
    ...
    public String getSignPromiseStatusColor(String project_id) {
        ...
        try {
            ...
            DB2Manager dbmsx = new DB2Manager();
            String query = "SELECT * FROM e_budget.specific_method_plan WHERE project_id = '" + project_id + "'";
            ResultSet rsx = dbmsx.GetDataAsResultSet(query);
            if (rsx.next()) {
                EndDatePlan = rsx.getString("SIGN_PROMISE_END");
                query = "SELECT * FROM e_budget.specific_method_result WHERE project_id='" + project_id + "'";
