...
public class SelectionMethodPlan {
    ...
    public boolean InsertSelectionMethodPlan(SelectionMethodPlan sp) {
        boolean ret = false;
        try {
            String query = "SELECT COUNT(*) AS cc FROM e_budget.selection_method_plan WHERE project_id='" + sp.getProject_id() + "'";
            DB2Manager dbms = new DB2Manager();
            ResultSet rs = dbms.GetDataAsResultSet(query);
            rs.next();
            if (rs.getInt("cc") > 0) {
                query = "UPDATE e_budget.selection_method_plan SET "
                ...
                query = "INSERT INTO e_budget.selection_method_plan(project_id,invitation_announcement_start,invitation_announcement_end,"
    ...
    public boolean DeleteSelectionMethodPlan(SelectionMethodPlan sp) {
        boolean ret = false;
        try {
            String query = "DELETE FROM e_budget.selection_method_plan WHERE project_id='" + sp.getProject_id() + "'";
            DB2Manager dbms = new DB2Manager();
            ret = dbms.Excute(query);
        } catch (Exception ex) {
            ret = false;
