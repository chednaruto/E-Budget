...
<%
    String tag = "";
    String budget_type_status = "";
    String budget_type_name = "";
    if (!acc.IsNullToEmtyString(request.getParameter("budget_type_id")).isEmpty()) {
        String query = "SELECT * FROM e_budget.budget_type WHERE budget_type_id=" + acc.IsNullToEmtyString(request.getParameter("budget_type_id"));
        DB2Manager mm = new DB2Manager();
        ResultSet rs = mm.GetDataAsResultSet(query);
        rs.next();
        if (acc.IsNullToEmtyString(rs.getString("budget_type_status")).equals("Y")) {
            budget_type_status = "checked='checked'";
        }
        ...
        <div class="main-content-inner">
            <div class="breadcrumbs" id="breadcrumbs">
                ...
                <ul class="breadcrumb">
                    ...
                    <li><a href="index.jsp?service=BudgetType"> ประเภทงบประมาณ </a></li>
                    <li> <%=tag%>ประเภทงบประมาณ </li>
                </ul>
            ...
            <div class="page-content">
                <div class="page-header">
                    <h1><small style="color:#4D96CB!important;"><i class="ace-icon fa fa-angle-double-right"></i> ประเภทงบประมาณ </small></h1>
                </div>
                <div class="row">
                    <div class="col-xs-12">
                        <div>
                                        ...
                                        <div class="width-80 label label-info label-xlg arrowed-in arrowed-in-right">
                                            <div class="inline position-relative">
                                                <b href="#" class="user-title-label dropdown-toggle" data-toggle="dropdown">
                                                    <span class="white"><%=tag%>ประเภทงบประมาณ</span>
                                                </b>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="profile-info-value col-sm-12">
                                                <span>
                                                    <div class="input-group col-sm-12">
                                                        <input class="form-control active" type="text" id="budget_type_tf" value="<%=budget_type_name%>">
                                            ...
                                            <div class="profile-info-value">
                                                <span id="username">
                                                    <div class="col-xs-12 center">
                                                        <label>
                                                            <input id="budget_type_status_tf" class="ace ace-switch ace-switch-6" type="checkbox" <%=budget_type_status%>>
...
<script>
    function Add() {
        waitingDialog.show();
        var budget_type_id = '<%=acc.IsNullToEmtyString(request.getParameter("budget_type_id"))%>';
        var budget_type_name = $('#budget_type_tf').val();
        if (budget_type_name.trim() == "") {
            $.confirm({
        ...
        var budget_type_status = $('#budget_type_status_tf').is(":checked") ? 'Y' : 'N';

        var budget_type = {
            budget_type_id: budget_type_id,
            budget_type_name: budget_type_name,
            budget_type_status: budget_type_status,
            METHOD: "INSERT"
        };
        $.ajax({
            type: "POST",
            url: "BudgetTypeServlet",
